export const getImportDefinition = (file_name, code) => {
    const regex_system = /#include\s*<([\w\/\.]+)>/g
    const regex_local = /#include\s*"([\w\/\.]+)"/g
    ...
    while ((match = regex_system.exec(code)) !== null) {
        if (match[1]) {
            const last_dot_index = match[1].lastIndexOf('.')
            let function_name = match[1].slice(0, last_dot_index)
            if (last_dot_index === -1) {
                function_name = match[1]
            }
            let function_file = 'System'
    ...
    // function_name: 
    while ((match = regex_local.exec(code)) !== null) {

        if (match[1]) {
            const last_dot_index = match[1].lastIndexOf('.')
            const last_slash_index = match[1].lastIndexOf('/')
            const last_second_slash_index = match[1].lastIndexOf('/', last_slash_index - 1)

            const function_name = match[1].slice(last_slash_index + 1, last_dot_index)
            
            let function_file = match[1].slice(last_second_slash_index + 1, last_slash_index)

            if (last_second_slash_index === -1) {
                function_file = match[1].slice(0, last_slash_index)
            }
